...
export default function IssuesPage() {
  ...
  const getPriorityBadge = (priority: string) => {
    const variants: Record<string, { color: string; label: string }> = {
      low: { color: 'bg-blue-600', label: 'Düşük' },
      medium: { color: 'bg-yellow-600', label: 'Orta' },
      high: { color: 'bg-orange-600', label: 'Yüksek' },
      critical: { color: 'bg-red-600', label: 'Kritik' },
    };
  ...
  const getSLAStatus = (issue: Issue) => {
    ...
    if (issue.status === 'resolved' || issue.status === 'closed') {
      return { status: 'completed', text: 'Tamamlandı', color: 'text-green-600' };
    }
    
    if (hoursLeft < 0) {
      return { status: 'overdue', text: `${Math.abs(hoursLeft)} saat gecikti`, color: 'text-red-600' };
    } else if (hoursLeft < 24) {
      return { status: 'urgent', text: `${hoursLeft} saat kaldı`, color: 'text-orange-600' };
    } else {
      const daysLeft = Math.floor(hoursLeft / 24);
      return { status: 'ok', text: `${daysLeft} gün kaldı`, color: 'text-blue-600' };
    }
  };
        ...
        <div className="max-w-[1800px] mx-auto space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
            ...
            <Card className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-blue-600">{localStats.open}</div>
            ...
            <Card className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-orange-600">{localStats.inProgress}</div>
            ...
            <Card className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-green-600">{localStats.resolved}</div>
                {stats && (
                  <p className="text-xs text-muted-foreground mt-1">
            ...
            <Card className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-purple-600">{localStats.myIssues}</div>
            ...
            <Card className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-red-600">{localStats.critical}</div>
            ...
            <Card className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-red-600">{localStats.overdue}</div>
                {stats && (
                  <p className="text-xs text-muted-foreground mt-1">
                    ...
                    <div>
                      ...
                      <p className="flex items-center gap-2">
                        ...
                        {(() => {
                          ...
                          return sla && (
                            <Badge className={sla.status === 'overdue' ? 'bg-red-600' : sla.status === 'urgent' ? 'bg-orange-600' : 'bg-blue-600'}>
                              {sla.text}
                            </Badge>
                          );
                        })()}
